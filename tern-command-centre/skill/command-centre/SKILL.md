...
| A deliverable exists (deck, report, record, app, vault change) | `generations/<date>-<slug>.md` with `agent`, `flow`, `path` or `url`, one-paragraph body |
...
4. Publish `dist/index.html` to the artifact `https://claude.ai/code/artifact/932a6090-6c9f-4d1c-81fe-f4e0bfa8252c` (same URL) and update the vault note `Projects/Tern OS Command Centre`.
5. Append a run to `flows/command-centre-refresh.json`.
